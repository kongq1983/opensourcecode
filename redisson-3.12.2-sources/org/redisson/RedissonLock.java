...
//        get(lockAsync(leaseTime, unit));
    ...
    <T> RFuture<T> tryLockInnerAsync(long leaseTime, TimeUnit unit, long threadId, RedisStrictCommand<T> command) {
        ...
        // KEYS[1] 加锁的那个key  getName()    ARGV[1] 锁key的续约时间，默认30秒  internalLockLeaseTime  ARGV[2]  代表的是加锁的set的vaule（id + ":" + threadId） getLockName(threadId)
        return commandExecutor.evalWriteAsync(getName(), LongCodec.INSTANCE, command,
                  "if (redis.call('exists', KEYS[1]) == 0) then " + // 分布式锁的key不存在
                      "redis.call('hset', KEYS[1], ARGV[2], 1); " + // 通过hset命令   设置ARGV[2]的值为1
                      "redis.call('pexpire', KEYS[1], ARGV[1]); " + //  通过pexpire设置KEYS[1]的失效时间
                      "return nil; " +
                  "end; " +
                  "if (redis.call('hexists', KEYS[1], ARGV[2]) == 1) then " +   // 分布式锁的KEY已经存在，并且value也匹配  可重入锁
                      "redis.call('hincrby', KEYS[1], ARGV[2], 1); " +  // 把当前线程持有锁数量+1
                      "redis.call('pexpire', KEYS[1], ARGV[1]); " +  // 设置失效时间
                      "return nil; " +
                  "end; " +
                  "return redis.call('pttl', KEYS[1]);", // 获取分布式锁的失效时间(毫秒)
...
//        throw commandExecutor.convertException(future);
    ...
    protected RFuture<Boolean> unlockInnerAsync(long threadId) { // KEYS[1]: redis的key    ARGV[2]:internalLockLeaseTime过期时间  ARGV[3]: id + ":" + threadId
        return commandExecutor.evalWriteAsync(getName(), LongCodec.INSTANCE, RedisCommands.EVAL_BOOLEAN,
                "if (redis.call('hexists', KEYS[1], ARGV[3]) == 0) then " + //分布式锁不存在
                    "return nil;" +
                "end; " +
                "local counter = redis.call('hincrby', KEYS[1], ARGV[3], -1); " +  //分布式锁存在 把持有锁的数量减1
                "if (counter > 0) then " + // 重入次数减1后的值如果大于0，表示分布式锁有重入过，那么只设置失效时间，目前还不能删除
                    "redis.call('pexpire', KEYS[1], ARGV[2]); " +
                    "return 0; " +
                "else " +   //重入次数减1后的值如果为0，表示分布式锁只获取过1次，那么删除这个KEY，并发布解锁消息
